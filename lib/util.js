...
exports.loadTestData = function (context, done) {
  console.log('loadTestData');
  context.data = {};
  context.testData.forEach(function (testdata) {
    var zone = testdata.zone;
    context.data[zone] = context.data[zone] || [];
    var record = {};
    testdata.data[0].beacons.forEach(function (beacon) {
      record[beacon.major + '-' + beacon.minor] = beacon.rssi;
    });
    context.data[zone].push(record);
  });
  done();
...
exports.loadModel = function (context, done) {
  fs.readFile(context.modelFile, function (err, model) {
    ...
    context.svmMapping = model.svm.beaconIDs;
    context.svmZones = model.svm.locationIDs;
