...
function getMaxZoneId(zoneMapping) {
  var max = 0;
  if (zoneMapping) forEach(zoneMapping, function (id) {
    if (id > max) max = id;
  });
  return max;
}
...
function invert(hash) {
  var res = {};
  forEach(hash, function (value, key) {
...
exports.writeTrainData = function (context, done) {
  ...
  context.svmMapping = context.svmMapping || {};
  var zoneMapping = context.svmZoneMapping ? invert(context.svmZoneMapping) : {};
  var out = fs.createWriteStream(context.tempTrainFile);
  var nextFeatureId = 0;
  var nextZoneId = 1;
  eachMeasurement(context.data, function (measurement, zone) {
    if (train && !zoneMapping[zone]) zoneMapping[zone] = nextZoneId++;
    if (!zoneMapping[zone]) return;
    var beacons = [];
    forEach(measurement, function (rssi, beacon) {
    ...
    out.write(zoneMapping[zone] + ' ' + beacons.map(function (x) {
      return x.join(':');
    }).join(' ') + '\n');
  });
  if (train) context.svmZoneMapping = invert(zoneMapping);
  out.end(done);
};
...
exports.writeOutput = function (context, done) {
  var output = {
    simple: {
      ...
      mapping: context.svmMapping,
      zoneMapping: context.svmZoneMapping,
      model: context.model
    }
  };
  fs.writeFile(context.modelFile, JSON.stringify(output, false, '  '), done);
};
...
exports.loadModel = function (context, done) {
  fs.readFile(context.modelFile, function (err, model) {
    ...
    context.svmZoneMapping = model.svm.zoneMapping;
      ...
      fs.readFile(context.tempTestFile, function (err, contents) {
        ...
        contents.forEach(function (row) {
          ...
          if (!context.svmProbHeader) {
            ...
            context.svmZone.push(context.svmZoneMapping[label]);
